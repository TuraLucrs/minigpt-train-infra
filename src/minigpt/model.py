...
class CausalSelfAttention(nn.Module):
    ...
    def prefill_with_cache(
        ...
        cache.key[rows, :, positions] = k[rows, :, tokens]
        cache.value[rows, :, positions] = v[rows, :, tokens]
    ...
    def decode_with_cache(
        ...
        if rows.numel():
            ...
            cache.key[rows, :, write_positions] = k[active_mask, :, 0]
            cache.value[rows, :, write_positions] = v[active_mask, :, 0]
...
class TransformerBlock(nn.Module):
    ...
    def prefill_with_cache(self, x, attention_mask, position_ids, cache):
        x = x + self.attn.prefill_with_cache(self.ln_1(x), attention_mask, position_ids, cache)
        return x + self.mlp(self.ln_2(x))

    def decode_with_cache(self, x, cache, positions, active_mask, new_lengths, max_length):
        x = x + self.attn.decode_with_cache(
            self.ln_1(x), cache, positions, active_mask, new_lengths, max_length
        )
        return x + self.mlp(self.ln_2(x))
...
class MiniGPT(nn.Module):
    ...
    def prefill_with_cache(
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor, cache: MiniGPTKVCache
    ) -> torch.Tensor:
        B, T = input_ids.shape
        if attention_mask.shape != input_ids.shape:
            raise ValueError("attention_mask 必须与 input_ids 同为 [B,T]")
        ...
        for block, layer_cache in zip(self.blocks, cache.layers):
            x = block.prefill_with_cache(x, attention_mask, position_ids, layer_cache)
        cache.lengths[:B].copy_(lengths)
        cache.current_max_length = int(lengths.max().item())
        return self.lm_head(self.ln_f(x))

    def decode_with_cache(
        self, input_ids: torch.Tensor, active_mask: torch.Tensor, cache: MiniGPTKVCache
    ) -> torch.Tensor:
        B = cache.batch_size
        if input_ids.shape != (B, 1) or active_mask.shape != (B,):
            raise ValueError("Decode 需要 [B,1] input_ids 和 [B] active_mask")
        positions = cache.lengths[:B].clone()
        ...
        for block, layer_cache in zip(self.blocks, cache.layers):
            x = block.decode_with_cache(
                x, layer_cache, positions, active_mask.bool(), new_lengths, max_length
            )
        cache.lengths[:B].copy_(new_lengths)
