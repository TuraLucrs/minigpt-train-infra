"""Qwen3 单请求、单设备 TTFT/TPOT/吞吐/显存 Benchmark。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="测量 Qwen3 单设备推理；正式 32B 性能需先通过显存门禁。")
    ...
    parser.add_argument("--decode-mode", choices=("compare", "kv_cache", "recompute"), default="kv_cache")
    parser.add_argument("--warmup", type=int, default=2)
    parser.add_argument("--repeats", type=int, default=5)
    parser.add_argument("--hash-weights", action="store_true", help="计算全部 shard SHA-256（32B 会很慢）")
    parser.add_argument("--output", default="runs/qwen3_single_device.json")
...
def main() -> None:
    ...
    recompute = InferenceEngine(
        RecomputeQwen3ModelRunner(model, runtime),
        tokenizer,
        pad_token_id=tokenizer.pad_token_id,
    )
    ...
    print("=" * 80)
    print("Qwen3 单设备推理 Benchmark（中位数）")
